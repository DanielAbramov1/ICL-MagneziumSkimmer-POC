...
class camFuncs:
    ...
    def readCamera(self):
        ...
        if(data_from_cam):
            data_from_cam = (self.tn.read_until(b'\r\n',1).decode('utf-8'))
            data_from_cam = data_from_cam.strip('\r\n')
            print("Cam (telnet) data received: " + data_from_cam)
    ...
    """read cam_q and send cmnds to cam via telnet"""
    def read_from_q(self):
        
        if(cam_q.empty() == False):
            self.tn.write((cam_q.get()).encode())
    
    """send manual cmnds to cam via dictionary"""
